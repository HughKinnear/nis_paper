...
def EMvMFNM(X, W, k, random_state):
    # reshaping just to be sure
    W = W.reshape(-1, 1)

    # initialization
    M = initialization(X, k, random_state)
...
# Initialization
# --------------------------------------------------------------------------
def initialization(X, k, random_state):

    # Random initialization
    n = np.size(X, axis=1)
    idx = random_state.choice(range(n), k)
    m = X[:, idx]
    label = np.argmax(
...
def maximization(X, W, R, M):
    ...
    kappa = np.where(np.isnan(kappa), 10, kappa)
    kappa = np.where(kappa <= 0, 10, kappa)
